<template>
   <div class="py-4 container-fluid">
        ...
        <div class="card-header pb-0">
            ...
            ></i>  <span class="ml-5">Add New Medical Technologist</span>
                </button>
            </div>
        </div>
    ...
    <div class="card mt-5">
        <div class="card-header pb-0">
            <h6>Medical Technologist List</h6>
        </div>
        <div class="card-body px-0 pt-0 pb-3 mt-2">
            ...
            <tbody>
                <tr v-for="(item) in this.$store.state.medtechList" :key="item">
                    <td>{{item.name}}</td>
                    <td>{{item.license}}</td>
...
methods:{
    ...
    {
      this.$store.dispatch("deleteRecord",{table:"Medtech",id:this.removeIndex,dispatchName:"getMedtech"})
      this.clearInput
    } },
    UpdateRecord(data){
    let medtechData = this.$store.getters.getMedtechById(data.id)
    this.mode = 1
    this.editId = data.id
    this.name = medtechData.name
    this.license = medtechData.license
    this.signatureImage = medtechData.signatureImage
},
    fileUpload($event){
    console.log($event)
    ...
    saveData()
{
    ...
    if(this.mode==0)
    {
        this.$store.dispatch('saveMedtech',data)
    }
    else
    {
        data.id = this.editId
        this.$store.dispatch('updateMedtech',data)
    }
   this.resetData()
},
    resetData(){
...
alert:{
  title: "Remove Medical Technologist",
  message: "Are you sure you want to remove this Medical Technologist ? "
}
        }
    }
